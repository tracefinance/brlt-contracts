...
// SubscriptionRegistry manages contract event subscriptions
...
func (r *SubscriptionRegistry) GetSubscription(chainType types.ChainType, contractAddr string) *ContractSubscription {
	r.mutex.RLock()
...
func (r *SubscriptionRegistry) AddOrUpdateSubscription(sub *ContractSubscription) {
	r.mutex.Lock()
...
func (r *SubscriptionRegistry) RemoveSubscription(chainType types.ChainType, contractAddr string) {
	r.mutex.Lock()
	defer r.mutex.Unlock()
...
func (r *SubscriptionRegistry) CancelAllSubscriptions() {
	r.mutex.Lock()
	defer r.mutex.Unlock()
...
func (r *SubscriptionRegistry) GetSubscriptionsForChain(chainType types.ChainType) []*ContractSubscription {
	r.mutex.RLock()
	defer r.mutex.RUnlock()
...
func (r *SubscriptionRegistry) HasEventSubscription(chainType types.ChainType, contractAddr string, eventSig string) bool {
	r.mutex.RLock()
	defer r.mutex.RUnlock()
